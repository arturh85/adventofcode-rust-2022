...
//! [magical energy](https://adventofcode.com/2018/day/25) to deliver presents on Christmas.
...
#[aoc_generator(day1)]
fn parse_input(input: &str) -> Result<Vec<Vec<usize>>, Box<dyn Error>> {
...
fn part2(elves: &[Vec<usize>]) -> usize {
    let mut elves: Vec<usize> = elves.iter().map(|elf | elf.iter().sum() ).collect();
    elves.sort();
    let l = elves.len();
...
10000";
    ...
    #[test]
    fn part1_examples() {
        assert_eq!(part1(&parse_input(EXAMPLE).unwrap()), 24000);
    }

    #[test]
    fn part2_examples() {
        assert_eq!(part2(&parse_input(EXAMPLE).unwrap()), 45000);
    }
}
